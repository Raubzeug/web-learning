...
var addCourseHandler = function(course) {
    ...
    infoButton.addEventListener('click', function(evt) {
        ...
        lectionList.classList.remove('hidden')
        infoButton.textContent = ('Скрыть список лекций')
        flag = true
        }
        else {
            evt.preventDefault()
            course.classList.remove('course_wrapper_full')
            lectionList.classList.add('hidden')
            infoButton.textContent = ('Посмотреть лекции курса')
            flag = false
        }
    })
}
